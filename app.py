...
app.layout = html.Div(
  [
    ...
    html.Img(id='logo', style={'display':'inline','height':'3%', 'width':'3%', 'margin-left':'2%','margin-bottom':'1%'}),
    html.Br(),
    dcc.Dropdown(
                id='stock',
                options=[ {'label': row['name'] + ' | ' + row['symbol'], 'value': row['symbol']} for index, row in all_stocks.iterrows()],
                value='AAPL'
            ),
    html.Br(),
    dbc.Alert(
...
def update_description(symbol):
    ...
    try:
        ...
        location = "{}, {}, {}, {}".format(profile['address'][0],profile['city'][0],profile['state'][0], profile['country'][0])
        locations = geocode(location, provider='nominatim', user_agent='my_app')
...
def update_stock(symbol,start,end):
    ...
    try:
        ...
        fig = px.line(x=df.index,y=df['Open'],labels={'y':'Price in $', 'x':''})
        fig.update_layout(
                    showlegend=False,
                    margin = dict(l = 0, r = 0, t = 0, b = 0),
                    height=300,
        ...
        avg = returns.mean()
        dist = px.histogram(returns, histnorm='probability density', labels={'value':'returns'})
        ...
        log_plot = px.line(x=df.index,y=df['Open'],labels={'y':'Log Price in $', 'x':''}, log_y=True)
        dist.update_layout(
                    showlegend=False,
                    margin = dict(l = 0, r = 0, t = 0, b = 0),
                    height=300,
